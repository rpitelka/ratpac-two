...
void WaveformAnalysisRSNNLS::DoAnalysis(DS::DigitPMT* digitpmt, const std::vector<UShort_t>& digitWfm) {
  ...
  for (const std::pair<int, int>& crossing : crossings) {
    ...
    for (int i = 0; i < w.size(); ++i) {
      if (w[i] > 0.0) {
        nonzero_indices.push_back(start_sample + i);
        nonzero_weights.push_back(w[i]);
      }
    }
  }
  ...
  for (size_t i = 0; i < nonzero_indices.size(); ++i) {
    double delay = nonzero_indices[i] * fTimeStep / fUpsampleFactor;
    fit_result->AddPE(delay, nonzero_weights[i]);
  }
}
...
PMTPulse WaveformAnalysisRSNNLS::MakeTemplate(DS::DigitPMT* digitpmt) {
  ...
  pulse.SetPulseCharge(fChargeMean * charge_scale);
...
Eigen::VectorXd WaveformAnalysisRSNNLS::Thresholded_rsNNLS(const Eigen::VectorXd& m, const Eigen::MatrixXd& S,
                                                           double threshold) {
  int D = m.size();
  int D2 = S.rows();
  int K = S.cols();
  ...
  while (!S_reduced.empty()) {
    ...
    if (S_reduced.empty()) return Eigen::VectorXd::Zero(K);
